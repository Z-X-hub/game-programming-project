...
| ADVENTURER double jump | The double jump needed to work only for ADVENTURER, without giving every character two jumps. | Used role checks and jump-count logic in `RedCharacter.cs`. | This made the ability clear and avoided changing the base movement rules for all characters. |
